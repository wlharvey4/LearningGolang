...
#+title:Learning the Golang Language
#+subtitle:{{{version}}} {{{date}}}
#+author:Wesley Harvey
#+date:2020-06-17 00:03
...
#+begin_src emacs-lisp
  (defun create-new-project (project title author &optional bucket)
    ...
    (message "PROJECT=%s TITLE=%s AUTHOR=%s BUCKET=%s" project title author bucket)
    (mkdir project t)
    (message "SYNC_ORG_TEMPLATE=%s" (getenv "SYNC_ORG_TEMPLATE"))
    (message "file-name=%s" project)
	...
	(re-search-forward "^#[+]author:\\(AUTHOR\\)$")
	(replace-match author t nil nil 1)
	(re-search-forward "^#[+]macro:version Version \\(.*\\)$")
...
#+header: :tangle /usr/local/dev/bin/run-org-template
...
#+begin_src sh
  ...
  curl -iv -H "Authorization: token ${GITHUB_TOKEN}" \
    -d "{\"name\":\"$1\"}" \
...
#+texinfo_printed_title:LearningGolang---Learning the Golang Language
